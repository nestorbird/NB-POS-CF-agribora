...
login.bind_events = function() {
    ...
    $(".form-login").on("submit", function(event) {
        ...
        // Store credentials temporarily for potential session refresh
        sessionStorage.setItem('temp_login_email', args.usr);
    ...
    var login_handlers = {
        200: function(data) {
            if (data.message == 'Logged In') {
                console.log('Login successful, starting redirect flow...');
                login.set_status({{ _("Success") | tojson }}, 'green');
                
                // Verify session and store user info
                frappe.call({
                    method: "frappe.auth.get_logged_user",
                    callback: function(r) {
                        if (r.message && r.message !== 'Guest') {
                            frappe.session.user = r.message;
                            sessionStorage.setItem('user_id', r.message);
                            document.cookie = `user_id=${r.message}; path=/; max-age=86400`;
                            
                            // Continue with splash screen
                            document.body.innerHTML = `{% include "templates/includes/splash_screen.html" %}`;
                            
                            // Role verification and routing
                            frappe.call({
                                ...
                                callback: function(response) {
                                    ...
                                    if (response.message && Array.isArray(response.message)) {
                                        // Check for Administrator role first
                                        if (response.message.includes("Administrator")) {
                                            window.location.href = "/app";
                                            return;
                                        }else{
                                            window.location.href = "/getpos-react";
                                            return;
                                        }                               
                                    }
                                },
                            ...
                            login.set_invalid({{ _("Session verification failed. Please try again.") | tojson }});
                            window.location.href = "/login";
                        }
                    },
                    error: function(err) {
                        console.error('Session verification error:', err);
                        login.set_invalid({{ _("Unable to verify session. Please try again.") | tojson }});
                        window.location.href = "/login";
                    }
                });
